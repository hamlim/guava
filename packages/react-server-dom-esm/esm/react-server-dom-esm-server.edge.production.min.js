/**
* @license React
 * react-server-dom-esm-server.edge.production.min.js
 *
...
function outlineModel(request, value) {
  request.pendingChunks++;
  const outlinedId = request.nextChunkId++; // We assume that this object doesn't suspend, but a child might.
...
function decodeAction(body, serverManifest) {
  ...
  body.forEach((value, key) => {
    ...
    if (key.startsWith('$ACTION_REF_')) {
      const formFieldPrefix = '$ACTION_' + key.slice(12) + ':'; // The data for this reference is encoded in multiple fields under this prefix.
...
function renderToReadableStream(model, moduleBasePath, options) {
  const request = createRequest(model, moduleBasePath, options ? options.onError : undefined, options ? options.context : undefined, options ? options.identifierPrefix : undefined, options ? options.onPostpone : undefined);
